...
describe("Test the Glossary Entry", () => {
    ...
    test("renders correctly normal Input", () => {
        const { getByTestId, getAllByTestId} = render(<GlossaryEntry expandedList={mockExpandedList} setExpandedList={ mockSetExpandedList} {...mockNorm} />);
    ...
    test("open GlossaryEntry", () => {
        const { getByRole } =render(<GlossaryEntry expandedList={mockExpandedList} setExpandedList={ mockSetExpandedList} {...mockNorm} />);
        fireEvent.click(getByRole("button"));
        expect(mockSetExpandedList).toHaveBeenCalled();
    });

    test("close GlossaryEntry", () => {
        const { getByRole } = render(<GlossaryEntry expandedList={mockExpandedList} setExpandedList={ mockSetExpandedList} {...mockTerm}/>)
        fireEvent.click(getByRole("button"));
        expect(mockSetExpandedList).toHaveBeenCalled();
    });
});