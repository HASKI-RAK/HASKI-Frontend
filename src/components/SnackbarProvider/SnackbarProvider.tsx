...
// import { CustomSnackbarProps } from '../CustomSnackbar/CustomSnackbar'
...
const SnackbarProvider: React.FC<React.PropsWithChildren> = ({children}) => {
    const [snackbarsErrorWarning, setSnackbarsErrorWarning] = useState<SnackbarProps[]>([])
    const [snackbarsSuccessInfo, setSnackbarsSuccessInfo] = useState<SnackbarProps[]>([])
    ...
    const removeSnackbar = (snackbarToRemove: SnackbarProps) => {
        if(snackbarToRemove.severity === 'error' || snackbarToRemove.severity === 'warning') {
            setSnackbarsErrorWarning((previous) => previous.filter((snackbar) => snackbar !== snackbarToRemove))
        }
        else if(snackbarToRemove.severity === 'success' || snackbarToRemove.severity === 'info') {
            setSnackbarsSuccessInfo((previous) => previous.filter((snackbar) => snackbar !== snackbarToRemove))
        }
    }

    const snackbarProviderValue = useMemo(() => (
        { snackbarsErrorWarning, snackbarsSuccessInfo, setSnackbarsErrorWarning, setSnackbarsSuccessInfo, addSnackbar, removeSnackbar }),
        [ snackbarsErrorWarning, snackbarsSuccessInfo, setSnackbarsErrorWarning, setSnackbarsSuccessInfo, addSnackbar, removeSnackbar]
    )

    return(
        <SnackbarContext.Provider
            value={snackbarProviderValue}
        >
            {children}
        </SnackbarContext.Provider>
    )
