...
// Component
const GlobalNavDropdown = ({ content, title, isLoading }: GlobalNavDropdown) => {
  // Hooks
          ...
          <Box sx={{ flexGrow: 0, ml: 1 }}>
            <Tooltip title={t('appGlobal.courseSelection')}>
              <Button
                ...
                sx={{ whiteSpace: 'pre-wrap', hyphens: 'manual' }}
                variant="text">
                {title.replaceAll(' ', '\n').replaceAll('&shy;', '-\n')}
              </Button>
            </Tooltip>
...
export default memo(GlobalNavDropdown)
