...
import {Box, Button, Modal, Stepper, Step, StepButton, IconButton, Stack, Typography, Link} from '@common/components'
...
const QuestionnaireResultsModal = ({
    ...
    useEffect(() => {
        if(activeStep === 1 && open === true) {
            fetchUser().then((user) => {
                return getListK(user.settings.user_id, user.lms_user_id, user.id).then((data) => {
                    setListKData(data)
                }).catch((error) => {
                    log.error(error)
    ...
    useEffect(() => {
        if(activeStep === 0 && open === true) {
            fetchUser().then((user) => {
                return getILS(user.settings.user_id, user.lms_user_id, user.id).then((data) => {
                    setILSData(data)
                }).catch((error) => {
                    log.error(error)
                    if(error.message !== 'Failed to fetch') {
