...
export const ILSDimension = (
  ...
  switch (dimensionNumber) {
    case 1:
      if (onlyEnglish) {
        const en = i18n.getFixedT('en')
        if (score > 0) return en('components.QuestionnaireResults.TableILS.Active')
        else return en('components.QuestionnaireResults.TableILS.Reflective')
      } else {
        if (score > 0) return t('components.QuestionnaireResults.TableILS.Active')
        else return t('components.QuestionnaireResults.TableILS.Reflective')
      }
    case 2:
      if (onlyEnglish) {
        const en = i18n.getFixedT('en')
        if (score > 0) return en('components.QuestionnaireResults.TableILS.Sensory')
        else return en('components.QuestionnaireResults.TableILS.Intuitive')
      } else {
        if (score > 0) return t('components.QuestionnaireResults.TableILS.Sensory')
        else return t('components.QuestionnaireResults.TableILS.Intuitive')
      }
    case 3:
      if (onlyEnglish) {
        const en = i18n.getFixedT('en')
        if (score > 0) return en('components.QuestionnaireResults.TableILS.Visual')
        else return en('components.QuestionnaireResults.TableILS.Verbal')
      } else {
        if (score > 0) return t('components.QuestionnaireResults.TableILS.Visual')
        else return t('components.QuestionnaireResults.TableILS.Verbal')
      }
    case 4:
      if (onlyEnglish) {
        const en = i18n.getFixedT('en')
        if (score > 0) return en('components.QuestionnaireResults.TableILS.Sequential')
        else return en('components.QuestionnaireResults.TableILS.Global')
      } else {
        if (score > 0) return t('components.QuestionnaireResults.TableILS.Sequential')
        else return t('components.QuestionnaireResults.TableILS.Global')
      }
    default:
