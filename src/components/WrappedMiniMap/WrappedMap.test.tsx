...
describe('WrappedMiniMap component', () => {
  ...
  it('resizes according to window size', () => {
    const {getByTestId} = render(<ReactFlowProvider><WrappedMiniMap /></ReactFlowProvider>)
    expect(getByTestId('rf__minimap')).toHaveStyle('transform: scale(0.75) translate(0rem, 0rem)')
    act(() => {
    ...
    expect(getByTestId('rf__minimap')).toHaveStyle('transform: scale(0.5) translate(0rem, 0rem)')
  })
})
