import { getLearningPathElement } from './getLearningPathElement'; // Replace with the correct module path
...
describe('getLearningPathElement has expected behaviour', () => {
  ...
  it('should throw an error when course_id or topic_id are missing', async () => {
    ...
    await expect(
        getLearningPathElement(userId, lmsUserId, studentId, courseId, topicId)
    ).rejects.toThrow('course_id and topic_id are required');
  });
  ...
  it('should throw a specific error when the response has an error variable', async () => {
    ...
    await expect(
        getLearningPathElement(userId, lmsUserId, studentId, courseId, topicId)
    ).rejects.toThrow(`${expectedError} ${expectedMessage}`);
  });
